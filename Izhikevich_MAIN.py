...
statemon = StateMonitor(source = neuronRS, variables = ['v', 'u', 'I'], record = True)
...
desired_range = []
for i in range(0, len(statemon.u[0])):
  if  -8*volt/second < statemon.u[0][i] < 0.5*volt/second:
...
for i in desired_range:
  u_range.append(statemon.u[0][i])
  vm_range.append(statemon.v[0][i])
...
statemon = StateMonitor(source = neuronIS, variables = ['v', 'u', 'I'], record = True)
...
statemon = StateMonitor(source = neuronCH, variables = ['v', 'u', 'I'], record = True)
...
#%%
def nullcine_model_creation(eq, solvar, plotvar, inputrange):
...
def solve_ODE_system(v_eequation_expression, u_equation_expression, x_range, y_range, x_var, y_var): 
  # Convert our equations to functions
  f1 = lambdify((x_var, y_var), v_eequation_expression)
  f2 = lambdify((x_var, y_var), u_equation_expression)
  ...
  quiver_one = [[f1(x_cur, y_cur) for x_cur in x1_range] for y_cur in y1_range];
  quiver_two = [[f2(x_cur, y_cur) for x_cur in x1_range] for y_cur in y1_range];
  # Solve analytically using sympy
  solutions = smp.solve([smp.Eq(v_eequation_expression, 0), smp.Eq(u_equation_expression, 0)], (x_var, y_var)) 
  # Calculate nullclines
  eq_one_nullcline = nullcine_model_creation(v_eequation_expression, y_var, x_var, total_range)
  eq_two_nullcline = nullcine_model_creation(u_equation_expression, y_var, x_var, total_range)

  # Get stable points and check their type
  stable_points = stable_point_function_for_analysis(v_eequation_expression, u_equation_expression, x_var, y_var, solutions)
  # Return relevant results
  return x_range, y_range, x1_range, y1_range, quiver_one, quiver_two, eq_one_nullcline, eq_two_nullcline, stable_points


def stable_point_function_for_analysis(v_eequation_expression, u_equation_expression, x_var, y_var, solutions):
  # First, calculate the jacob matrix for our equations
  eq_mat = Matrix([v_eequation_expression, u_equation_expression])
  var_mat = Matrix([x_var, y_var])
  jacob= eq_mat.jacobian(var_mat)
  ...
  for stable_point in solutions:
    ...
    eqmat = jacob.subs([(x_var, stable_point[0]), (y_var, stable_point[1])])
    ...
    if eigenvalues[0].is_real:
        if eigenvalues[0] > 0 and eigenvalues[1] > 0:
            definition_of_saddle_point = 'Unstable Node'
        elif eigenvalues[0] < 0 and eigenvalues[1] < 0:
            definition_of_saddle_point = 'Stable Node'
        elif (eigenvalues[0] < 0 and eigenvalues[1] > 0) or (eigenvalues[0] > 0 and eigenvalues[1] < 0):
            definition_of_saddle_point = 'Saddle Point'
    else:
        if eigenvalues[0].args[0] > 0:
            definition_of_saddle_point = 'Unstable Focus'
        if eigenvalues[0].args[0] < 0:
            definition_of_saddle_point = 'Stable Focus'
    # Add tuple for each stable point to list
    stable_points.append((stable_point, definition_of_saddle_point))
  # Return list of stable points
  return stable_points


def plot_iz_system(xlimit, ylimit,x_range, y_range, x1_range, y1_range, quiver_one, quiver_two, eq_one_nullcline, eq_two_nullcline, stable_points, x_behavior, y_behavior, x_var_name, y_var_name):
  ...
  # Plot quivers
  ax.quiver(x1_range, y1_range, quiver_one, quiver_two, alpha=.5, headwidth = 3, headlength = 2, headaxislength = 2)
  # Plot nullclines
  [ax.plot(x_range, nullcline, c = 'b', alpha=.7, label= x_var_name + ' Nullcline') for nullcline in eq_one_nullcline]
  [ax.plot(x_range, nullcline, c = 'r', alpha=.7, label= y_var_name + ' Nullcline') for nullcline in eq_two_nullcline]
  # Plot stable points
  for stable_point in stable_points:
    # Extract information
    x_stability = stable_point[0][0]
    y_stability = stable_point[0][1]
    definition_of_saddle_point = stable_point[1]
    ...
    try: # We have to use try to ignore errors that occur when trying to plot complex value
      if definition_of_saddle_point == 'Unstable Node':
        ax.scatter(x_stability, y_stability, marker = '.', label = definition_of_saddle_point, s = 150, c = 'coral')
      elif definition_of_saddle_point == 'Stable Node':
        ax.scatter(x_stability, y_stability, marker = '.', label = definition_of_saddle_point, s = 150, c = 'cyan')
      elif definition_of_saddle_point == 'Saddle Point':
        ax.scatter(x_stability, y_stability, marker = '.', label = definition_of_saddle_point, s = 150, c = 'violet')
      elif definition_of_saddle_point == 'Unstable Focus':
        ax.scatter(x_stability, y_stability, marker = '.', label = definition_of_saddle_point, s = 150, c = 'orange')
      elif definition_of_saddle_point == 'Stable Focus':
        ax.scatter(x_stability, y_stability, marker = '.', label = definition_of_saddle_point, s = 150, c = 'springgreen')  
    except:
...
def define_neuron_network(v_max):
...
def plot_IDS(boolean, xlimit, ylimit,I_ext, a_input, b_input, c_input, d_input, v_max):
  ...
  start_scope()
  # Define the neuron
  neuron = define_neuron_network(v_max)      
  ...
  v, u = symbols('v u')
  # First expression
  v_eequation_expression = I_ext + 0.04*v**2 + 5*v + 140 - u
  # Second expression
  u_equation_expression = a_input*((b_input*v) - u)  

  # Solve dynamical system
  x_range, y_range,x1_range, y1_range, quiver_one, quiver_two, eq_one_nullcline, eq_two_nullcline, stable_points = solve_ODE_system(v_eequation_expression, u_equation_expression, x_range = [-160, 60] , y_range = [-65, 65], x_var = v, y_var = u)
  xlim = xlimit
  ylim = ylimit
  # Plot results
  plot_iz_system(xlim, ylim, x_range, y_range,x1_range, y1_range, quiver_one, quiver_two, eq_one_nullcline, eq_two_nullcline, stable_points, x_behavior = statemon.v[0]/mV, y_behavior = statemon.u[0], x_var_name = 'v', y_var_name = 'u')
...
plotone = plot_IDS("True",x_limit,y_limit, I_ext_def, a_def, b_def, c_def, d_def, vmax_def)
#%%
#a zoomed in more detailed view
x_limit = [-80, 35]
y_limit = [-7, -2]

plotone = plot_IDS("false",x_limit,y_limit, I_ext_def, a_def, b_def, c_def, d_def, vmax_def)
...
# %%
#------------------------------------playing_around------------------------------------
#trying to find a stable state for an example
